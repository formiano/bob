...
class SecondInfoBar(Screen):
    ...
    def __init__(self, session):
        ...
        self.skinName = config.usage.show_second_infobar.value
...
class InfoBarShowHide(InfoBarScreenSaver):
    ...
    def toggleShow(self):
        ...
        if not self.LongButtonPressed:
            if self.__state == self.STATE_HIDDEN:
                if not self.secondInfoBarWasShown or config.usage.show_second_infobar.value == 'EventInfo' and not self.EventViewIsShown:
                    self.show()
                if self.secondInfoBarScreen:
                    self.secondInfoBarScreen.hide()
                self.secondInfoBarWasShown = False
                self.EventViewIsShown = False
            elif self.secondInfoBarScreen and config.usage.show_second_infobar.value not in 'EventInfo' and not self.secondInfoBarScreen.shown:
                self.SwitchSecondInfoBarScreen()
                self.hide()
                self.secondInfoBarScreen.show()
                self.secondInfoBarWasShown = True
                self.startHideTimer()
            elif (config.usage.show_second_infobar.value == 'EventInfo' or isMoviePlayerInfoBar(self)) and not self.EventViewIsShown:
                self.hide()
                try:
                    self.openEventView()
                except:
...
class InfoBarChannelSelection():
    ...
    def __init__(self):
        ...
        self['ChannelSelectActions'] = HelpableActionMap(self, 'InfobarChannelSelection', {'switchChannelUp': (self.UpPressed, _('Open service list and select previous channel')),
...
class InfoBarSeek():
    ...
    def isSeekable(self):
        ...
        if self.getSeek() is None or isStandardInfoBar(self) and not self.timeshiftEnabled() and name == False and str(url).find('mkv') >= 0 or isStandardInfoBar(self) and not self.timeshiftEnabled() and name == False and str(url).find('flv') >= 0 or isStandardInfoBar(self) and not self.timeshiftEnabled() and name == False and str(url).find('mp4') >= 0:
            return False
        else:
            return True
...
class InfoBarTimeshiftState(InfoBarPVRState):
    ...
    def _mayShow(self):
        ...
        if self.shown and self.timeshiftEnabled() and self.isSeekable():
            ...
            self.pvrStateDialog.show()
        elif self.isSeekable() and name == True and url.find('mkv') >= 0 or self.isSeekable() and name == True and url.find('flv') >= 0 or self.isSeekable() and name == True and url.find('mp4') >= 0:
            InfoBarTimeshift.ptsSeekPointerSetCurrentPos(self)
            if config.timeshift.showinfobar.value:
    ...
    def __timeshiftEventName(self, state):
        ...
        elif str(url).find('mkv') >= 0 or str(url).find('flv') >= 0 or str(url).find('mp4') >= 0:
...
class InfoBarExtensions():
    ...
    def getCCcamInfo(self):
        if pathExists('/usr/emuscript/'):
            softcams = os.listdir('/usr/emuscript/')
...
#ATV
    def __init__(self):
        self['INFOpanelActions'] = HelpableActionMap(self, 'InfoBarINFOpanel', {'ItRedKey': (self.selectRedKeytask, _('Red Key')),
        #ATV
         'ItRedKeyLong': (self.doRedKeyTaskLong, _('Red Key Long'))})
...
class InfoBarQuickMenu():
    ...
    def __init__(self):
        self['QuickMenuActions'] = HelpableActionMap(self, 'InfoBarQuickMenu', {'quickmenu': (self.bluekey_qm, _('Quick Menu...'))})
...
class InfoBarInstantRecord():
    ...
    def recordQuestionCallback(self, answer):
        if answer is None or answer[1] == 'no':
            ...
            #VOD ITALY = (ATV) DOCO ATV PERCHE InfoBarGenerics E DELLA ATV!!! SI AVETE CAPITO BENE ITALYSAT E UN CLONE DELLA ATV E SE LA FANNO ANCHE PAGARE!!!!!! 
                self.saveTimeshiftEventPopupActive = False
                name = ServiceReference(self.session.nav.getCurrentlyPlayingServiceReference()).getServiceName()
                url = ServiceReference(self.session.nav.getCurrentlyPlayingServiceReference()).getPath()
                from ItalySat.ItalysatJobs import DownloadJob, JobManager
                jm = JobManager()
                if url:
                    Title = name
                    self.currentDownload = ''
                    self.currentDownload = jm.addJobToList(url, Title.replace(' ', '_'))
                    jm.startDownload(self.currentDownload)
                    self.session.open(MessageBox, _('Downloading starded - VOD: ' + Title.replace('_', ' ')), MessageBox.TYPE_INFO, timeout=5)
                    #VOD ITALY (ATV)
                else:
    ...
    def instantRecord(self, serviceRef = None):
        ...
        if not findSafeRecordPath(pirr) and not findSafeRecordPath(defaultMoviePath()):
            ...
            return
        if isStandardInfoBar(self):
            commonVOD = ((_('Download (remember to switch to a channel DVB-S2/T/T2)'), 'downloadvod'), (_('Add recording (stop after current event)'), 'event'))
            #VOD ITALY (ATV)
